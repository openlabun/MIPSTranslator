...
import {
  ...
  ParseResult,
} from '../Shared/Services/AssemblyParser/assembly-parser.service';
...
export class MainPageComponent {
  ...
  detectInstructionType(input: string): void {
    ...
    if (
      !this.translator.isValidHex(input) &&
      !this.translator.isValidMIPS(input)
    ) {
  ...
  onTranslate(): void {
    ...
    try {
      if (this.isHexToMIPS) {
        MIPS = this.translator.translateHextoMIPS(this.inputText, true);
        HEX = this.inputText.startsWith('0x')
          ? this.inputText.substring(2).toUpperCase()
          : this.inputText.toUpperCase();
        HEX = HEX.padStart(8, '0');
        if (MIPS.includes('Unknown') || MIPS.includes('Invalid'))
          throw new Error(MIPS);
      } else {
        HEX = this.translator.translateMIPStoHex(this.inputText);
        MIPS = this.inputText;
        if (
          HEX.includes('Unknown') ||
          HEX.includes('Invalid') ||
          HEX.includes('Missing')
        )
          throw new Error(HEX);
      }
      ...
      if (
        this.editingStackIndex >= 0 &&
        this.editingStackIndex < this.translations.length
      ) {
  ...
  private updateTableAfterStackEdit(translation: Translation): void {
    ...
    const instructionToShow = this.isHexToMIPS
      ? translation.hex
      : translation.mips;
  ...
  loadMipsCode(assemblyCode: string): void {
    ...
    if (!assemblyCode || assemblyCode.trim() === '') {
      this.parsingErrors = [
        'Por favor, ingresa código ensamblador en el área de texto.',
      ];
      return;
    }
    ...
    try {
      const result: ParseResult =
        this.assemblyParser.parseAssembly(assemblyCode);
      ...
      for (let i = 0; i < result.instructions.length; i++) {
        ...
        try {
          const hexInstruction =
            this.translator.translateMIPStoHex(mipsInstruction);

          if (
            hexInstruction.includes('Unknown') ||
            hexInstruction.includes('Invalid') ||
            hexInstruction.includes('Missing') ||
            hexInstruction.includes('Unsupported') ||
            hexInstruction.includes('Error')
          ) {
            translationErrors.push(
              `Línea ${i + 1} ('${mipsInstruction}'): ${hexInstruction}`,
            );
          } else {
          ...
          translationErrors.push(
            `Línea ${i + 1} ('${mipsInstruction}'): Error interno de traducción - ${e.message}`,
          );
        }
      }
      ...
      if (translationErrors.length > 0) {
        this.parsingErrors = [
          'Errores durante la traducción MIPS a HEX:',
          ...translationErrors,
        ];
        console.error(
          'Errores durante la traducción MIPS a HEX:',
        ...
        this.parameter = this.translations.map((t) => t.hex).join('\n');
        console.log(
          "Instrucciones cargadas en la lista 'translations':",
          this.translations,
        );
        alert(
          `¡${this.translations.length} instrucciones cargadas exitosamente en la pila de control!`,
        );
      }
    } catch (error: any) {
      console.error('Error inesperado durante el parseo:', error);
      this.parsingErrors = [
        `Error inesperado al procesar: ${error.message || 'Error desconocido'}`,
      ];
    }
  }

  onInstructionMenuSelect(event: {
    instruction: string;
  ...
  onReorderInstructions(reorderedTranslations: Translation[]): void {
    ...
    if (this.editingStackIndex >= 0) {
      ...
      this.editingStackIndex = reorderedTranslations.findIndex(
        (t) =>
          t.mips === editingTranslation.mips &&
          t.hex === editingTranslation.hex,
      );
      console.log(
        `Elemento en edición movido al nuevo índice: ${this.editingStackIndex}`,
      );
    }

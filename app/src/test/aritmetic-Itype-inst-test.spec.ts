...
describe('TranslatorService aritmetic I-type instruction tests', () => {
  ...
  describe('Translate I-type instruction to hex (LEGACY)', () => {
    beforeEach(() => {
      service.setVersion('legacy'); // ← AGREGAR ESTO
    });
  ...
  describe('Translate hex to I-type instructions (LEGACY)', () => {
    beforeEach(() => {
      service.setVersion('legacy'); // ← AGREGAR ESTO
    });
  ...
  describe('R6 should NOT have ADDI', () => {
    beforeEach(() => {
      service.setVersion('r6'); // ← R6 no tiene ADDI
    });

    it('should reject ADDI in R6', () => {
      const result = service.translateInstructionToHex('addi $t1 $t2 100');
      expect(result).toContain('Unknown instruction');
    });

    it('should still support ADDIU in R6', () => {
      const result = service.translateInstructionToHex('addiu t1 t2 10');
      expect(result).toEqual('2549000A');
    });
  });

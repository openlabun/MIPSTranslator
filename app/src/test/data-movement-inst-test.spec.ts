...
describe('TranslatorService I-type instruction tests', () => {
  ...
  describe('Translate data movement instructions to hex (LEGACY only)', () => {
    beforeEach(() => {
      service.setVersion('legacy'); // MFHI, MFLO, MTHI, MTLO solo en LEGACY
    });
  ...
  describe('Translate hex to data movement (LEGACY only)', () => {
    beforeEach(() => {
    ...
    it('should translate "01200011" to mips', () => {
  ...
  describe('R6 should NOT have HI/LO instructions', () => {
    beforeEach(() => {
      service.setVersion('r6');
    });

    it('should reject MFHI in R6', () => {
      const result = service.translateInstructionToHex('mfhi t1');
      expect(result).toContain('Unknown instruction');
    });

    it('should reject MFLO in R6', () => {
      const result = service.translateInstructionToHex('mflo t1');
      expect(result).toContain('Unknown instruction');
    });

    it('should reject MTHI in R6', () => {
      const result = service.translateInstructionToHex('mthi t1');
      expect(result).toContain('Unknown instruction');
    });

    it('should reject MTLO in R6', () => {
      const result = service.translateInstructionToHex('mtlo t1');
      expect(result).toContain('Unknown instruction');
    });
  });

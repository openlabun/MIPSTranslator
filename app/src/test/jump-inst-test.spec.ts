...
describe('TranslatorService jump instruction tests', () => {
  ...
  describe('Translate jump instructions to hex (LEGACY)', () => {
    beforeEach(() => {
  ...
  describe('Translate hex to jump instructions (LEGACY)', () => {
    beforeEach(() => {
      service.setVersion('legacy');
    });
  ...
  describe('R6 should NOT have JR (use JALR with $zero instead)', () => {
    beforeEach(() => {
      service.setVersion('r6');
    });

    it('should reject JR in R6', () => {
      const result = service.translateInstructionToHex('jr t1');
      expect(result).toContain('Unknown instruction');
    });

    it('should still support JALR in R6', () => {
      const result = service.translateInstructionToHex('jalr t1');
      expect(result).toEqual('0120F809');
    });

    it('should still support J in R6', () => {
      const result = service.translateInstructionToHex('j 1024');
      expect(result).toEqual('08000400');
    });

    it('should still support JAL in R6', () => {
      const result = service.translateInstructionToHex('jal 2048');
      expect(result).toEqual('0C000800');
    });
  });
